...
export class ProvenanceService {
  ...
  public deckEducation: ProvenanceSlidedeck;
  public application: Application;

  public textReport: String;
  public initialized = false;
  public findingCoord: any[] = [];
      ...
      this.http.post<Provenance>(`${environment.apiUrl}/provGraphs/provenance`,
        {
          ...
          IDcreator: IDcreator,
          findingCoord: this.findingCoord,
  ...
  public loadStory(storyInput: any) {
    ...
    this.slideDeck.setDeck((window as any).deck.restoreSelf(dataStory, this.traverser, this.graph, this.application));
    this.slideDeck._slidesInDeck = dataStory.slides.lenght;
  ...
  fission() {
      ...
      this.tracker.applyAction(action, true, parameters.artifactsParam.artifacts, 'split');
  }
  ...
  merging(currentNode: StateNode, nodeTo: StateNode) {
    ...
    this.settings.canvas.renderers2D.forEach(renderer => renderer._measurements.forEach(measurement => newBranchArtifacts.push(measurement.artifact)));
  ...
  newGraphComparison(graph?: ProvenanceGraph) {
    ...
    this.slideDeck = (window as any).slideDeck;
    this.slideDeck.setDeck(this.deck);
    this.slideDeck.update();
  ...
  newGraphEducation(graph?: ProvenanceGraph) {
    ...
    this.deckEducation = new ProvenanceSlidedeck(this.application, this.traverserEducation);

    this.slideDeck = (window as any).slideDeck;
    this.slideDeck.setDeck(this.deckEducation);
    this.slideDeck.update();
