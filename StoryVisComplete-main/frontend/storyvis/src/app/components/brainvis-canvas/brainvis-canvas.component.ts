...
export class BrainvisCanvasComponent extends THREE.EventDispatcher implements OnInit {
  ...
  private multiplePlaneIndex1: number;
  private multiplePlaneIndex2: number;
  ...
  createOneView(viewID: string) {
    ...
    this.activeRenderers.forEach(renderer => renderer.domElement.setAttribute('class', 'rendererNone'));
  ...
  removeOneView(viewID: string) {
    ...
    this.activeRenderers.forEach(renderer => renderer.domElement.setAttribute('class', 'renderer'));
  ...
  restoreGraph(input) {
    ...
    this.settings.loadedData = true;

    this.settings.canvas._measurements = [];
    this.settings.canvas.renderers2D.forEach((renderer) => {
      renderer._measurements = [];
      renderer._artifacts = [];
    });
    }
  ...
  prepareMultipleScenes() {
    ...
    const currentIndex = this.renderersMultiplePlane2D[0].stackHelper.index;

    if (!this.settings.multiplePlanesModeOn) {
      this.multiplePlaneIndex1 = this.renderersMultiplePlane2D[1].stackHelper.index;
      this.multiplePlaneIndex2 = this.renderersMultiplePlane2D[2].stackHelper.index;
    }
    ...
    if (this.settings.multiplePlanesModeOn) {
      this.setSliceIndexMultiplePlanes(currentIndex, 10);
      // this.activeRenderers.forEach(renderer => renderer.addEventListeners());
      this.animate();
      this.resize();
    } else {
      this.renderersMultiplePlane2D[0].stackHelper.index = currentIndex;
      this.renderersMultiplePlane2D[1].stackHelper.index = this.multiplePlaneIndex1;
      this.renderersMultiplePlane2D[2].stackHelper.index = this.multiplePlaneIndex2;
    }
  }


  multiplePlanes(viewID: string) {
    let sliceOrientation: string;
  ...
  removeArtifacts() {
    this.renderers2D.forEach(renderer => {
      renderer._measurements.forEach(measurement => {
      if (renderer._measurements.length !== 0) {
        renderer.removeArtifact(measurement.artifact);
      }
    })
  });
  }
  ...
  configParam() {
    ...
    this.renderers2D.forEach(renderer => renderer._measurements.forEach(
      measurement => artifactsParam.artifacts.push(measurement.artifact)));
    ...
    const parameters = { artifactsParam, locationParam, WLParam, magnificationParam };
  ...
  setConfig(parameters) {
    ...
    if (parameters.artifactsParam.toRemove) {
      this.removeArtifacts();
      parameters.artifactsParam.toRemove = false;
    } else {
      this.restoreArtifacts(parameters.artifactsParam.artifacts);
      parameters.artifactsParam.toRemove = true;
    }
  ...
  removeMeasurementsMP(indexMP1: number, indexMP2: number) {
    this.renderersMultiplePlane2D[1]._measurements = this.renderersMultiplePlane2D[0]._measurements;
    this.renderersMultiplePlane2D[2]._measurements = this.renderersMultiplePlane2D[0]._measurements;
    this.renderersMultiplePlane2D[1]._artifacts = this.renderersMultiplePlane2D[0]._artifacts;
    this.renderersMultiplePlane2D[2]._artifacts = this.renderersMultiplePlane2D[0]._artifacts;
    this.renderersMultiplePlane2D[1].removeMeasurementsMP(indexMP1);
    this.renderersMultiplePlane2D[2].removeMeasurementsMP(indexMP2);
  }

  renderMeasurementsMP(indexMP1: number, indexMP2: number) {
    this.renderersMultiplePlane2D[1].renderMeasurementsMP1(indexMP1);
    this.renderersMultiplePlane2D[2].renderMeasurementsMP2(indexMP2);
  }
  ...
  updateSliceIndexMultiplePlanesMinus(domID: string) {
    if (this.renderersMultiplePlane2D[0].domElement.id === domID) {
      if (this.renderersMultiplePlane2D[1].stackHelper.index - 1 > 1) {
        this.removeMeasurementsMP(this.renderersMultiplePlane2D[1].stackHelper.index, this.renderersMultiplePlane2D[2].stackHelper.index);
        ...
        this.renderMeasurementsMP(this.renderersMultiplePlane2D[1].stackHelper.index, this.renderersMultiplePlane2D[2].stackHelper.index);
      }
    } else if (this.renderersMultiplePlane2D[1].domElement.id === domID) {
  ...
  navigateVolume(sliceOrientation: VIEWS, newIndex: number, transitionTime: number) {
    this.settings.navigationMode = true;
    if (sliceOrientation === 'axial') {
  ...
  changeSliceRemove(sliceOrientation: VIEWS, oldIndex: number) {
    if (this.settings.multiplePlanesModeOn) {
      this.renderersMultiplePlane2D[1].removeFromSliceChange(oldIndex);
      this.renderersMultiplePlane2D[2].removeFromSliceChange(oldIndex);
    }
    if (sliceOrientation === 'axial') {
  ...
  changeSliceRender(sliceOrientation: VIEWS, newIndex: number) {
    if (this.settings.multiplePlanesModeOn) {
      this.renderersMultiplePlane2D[1].renderFromSliceChange(newIndex);
      this.renderersMultiplePlane2D[2].renderFromSliceChange(newIndex);
    }
    if (sliceOrientation === 'axial') {
