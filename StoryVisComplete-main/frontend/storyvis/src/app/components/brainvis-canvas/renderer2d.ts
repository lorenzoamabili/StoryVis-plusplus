...
export class Renderer2D extends AMIRenderer implements IAMIRenderer {
  ...
  public createdMeasurementsMP1: number = 0;
  public createdMeasurementsMP2: number = 0;

  public createArtifactMP: boolean = false;
  ...
  changeSliceIndex(newIndex: number, milliseconds: number, oldIndex?: number, done?: () => void) {
    ...
    if (milliseconds <= 0) {
      ...
      changeTimeout = setInterval((fromSlice, toSlice) => {
        ...
        if (this.settings.navigationMode) {
  ...
  removeFromSliceChange(oldIndex: number) {
    if (this._artifacts.find((artifact) => artifact.sliceIndex ? artifact.sliceIndex === oldIndex : ((artifact.sliceIndexStart - oldIndex) * (artifact.sliceIndexEnd - oldIndex)) <= 0)) {
      this._artifacts.filter((artifact) => artifact.sliceIndex ? artifact.sliceIndex === oldIndex : ((artifact.sliceIndexStart - oldIndex) * (artifact.sliceIndexEnd - oldIndex)) <= 0).forEach(
        (artifact) => {
          if (artifact.recreated === false) {
            this.removeElms(artifact);
          } else {
            this.removeRecreatedArtifact(artifact);
          }
        }
      )
    }
  ...
  renderFromSliceChange(newIndex: number) {
    if (this._artifacts.find((artifact) => artifact.sliceIndex ? artifact.sliceIndex === newIndex : ((artifact.sliceIndexStart - newIndex) * (artifact.sliceIndexEnd - newIndex)) <= 0)) {
      this._artifacts.filter((artifact) => artifact.sliceIndex ? artifact.sliceIndex === newIndex : ((artifact.sliceIndexStart - newIndex) * (artifact.sliceIndexEnd - newIndex)) <= 0).forEach(
        (artifact) => {
          // console.log(artifact.education);
          ...
          if (artifact.recreated === false) {
            ...
            this.recreateArtifact(artifact);
          }
        }
        // }
      )
  ...
  addArtifact(artifact: Artifact) {
    if (artifact.recreated === false && !this.settings.loadedData) {
      this.renderElms(artifact);
    } else if (this._canvas.settings.rulerMode || this._canvas.settings.angleMode ||
      this._canvas.settings.voxelprobeMode || this._canvas.settings.annotationMode) {
      this.renderElms(artifact);
    } else {
    ...
    const that = this;
    artifact.elements.forEach((elem) =>
      elem.addEventListener("contextmenu", (e) => {
        if (e.altKey) {
          const artifactToDelete = that._measurements.find((measurement) =>
            measurement.artifact.elements.includes(e.target as any)
          );
          that.removeArtifact(artifactToDelete.artifact);
          that.artifactDeleted.emit(artifactToDelete.artifact);
        }
      })
    );
    this._artifactInit = true;
    this._artifacts.push(artifact);
    this.settings.canvas._artifacts.push(artifact);
  };


  removeArtifact(artifactToRemove: Artifact) {
    if (artifactToRemove.recreated === false && !this.settings.loadedData) {
      this.removeElms(artifactToRemove);
    } else if (this._canvas.settings.rulerMode || this._canvas.settings.angleMode ||
      this._canvas.settings.voxelprobeMode || this._canvas.settings.annotationMode) {
      this.removeElms(artifactToRemove);
    } else {
      this.removeRecreatedArtifact(artifactToRemove);
    }
  ...
  saveCoordinates(artifact: Artifact) {    //not necessary, but make the data collection easier
    ...
    this._canvas.provenance.findingCoord.push(this.findingCoord);
  }

  recreateArtifact(artifact: Artifact) {
    // this._measurements
    // .find((measurement) => measurement.artifact.id === artifact.id).widget.show();
    this.settings.canvas._measurements
      .find((measurement) => measurement.artifact.id === artifact.id).artifact.elements.forEach((element) => {
        this.domElement.appendChild(element);
      });
  }

  removeRecreatedArtifact(artifactToRemove: Artifact) {
    this.settings.canvas._measurements
      //  this._measurements
      .find((measurement) => measurement.artifact.id === artifactToRemove.id)
      .artifact.elements.forEach((element) => {
  ...
  renderMeasurementsMP1(index: number) {
    if (this.settings.canvas._measurements.find((measurement) => measurement.artifact.sliceIndex ? measurement.artifact.sliceIndex === index : ((measurement.artifact.sliceIndexStart - index) * (measurement.artifact.sliceIndexEnd - index)) <= 0)) {
      this.settings.canvas._measurements.filter((measurement) => measurement.artifact.sliceIndex ? measurement.artifact.sliceIndex === index : ((measurement.artifact.sliceIndexStart - index) * (measurement.artifact.sliceIndexEnd - index)) <= 0).forEach(
        (measurement) => {
          if (this.createdMeasurementsMP1 < this._measurements.length) {
            this.createArtifactMP = true;
            this.createArtifact(measurement.artifact);
            this.createArtifactMP = false;
            this.createdMeasurementsMP1 = this.createdMeasurementsMP1 + 1;
          } else {
            this.recreateArtifact(measurement.artifact);
          }
        }
      )
    }
  ...
  renderMeasurementsMP2(index: number) {
    if (this.settings.canvas._measurements.find((measurement) => measurement.artifact.sliceIndex ? measurement.artifact.sliceIndex === index : ((measurement.artifact.sliceIndexStart - index) * (measurement.artifact.sliceIndexEnd - index)) <= 0)) {
      this.settings.canvas._measurements.filter((measurement) => measurement.artifact.sliceIndex ? measurement.artifact.sliceIndex === index : ((measurement.artifact.sliceIndexStart - index) * (measurement.artifact.sliceIndexEnd - index)) <= 0).forEach(
        (measurement) => {
          if (this.createdMeasurementsMP2 < this._measurements.length) {
            this.createArtifactMP = true;
            this.createArtifact(measurement.artifact);
            this.createArtifactMP = false;
            this.createdMeasurementsMP2 = this.createdMeasurementsMP2 + 1;
          } else {
            this.recreateArtifact(measurement.artifact);
          }
        }
  ...
  removeMeasurementsMP(index: number) {
    if (this.settings.canvas._measurements.find((measurement) => measurement.renderer._domID === ('r00' || 'r000' || 'r22' || 'r222' || 'r33' || 'r333')) &&
      this.settings.canvas._measurements.find((measurement) => measurement.artifact.sliceIndex ? measurement.artifact.sliceIndex === index : ((measurement.artifact.sliceIndexStart - index) * (measurement.artifact.sliceIndexEnd - index)) <= 0)) {
      this.settings.canvas._measurements.filter((measurement) => measurement.artifact.sliceIndex ? measurement.artifact.sliceIndex === index : ((measurement.artifact.sliceIndexStart - index) * (measurement.artifact.sliceIndexEnd - index)) <= 0).forEach(
        (measurement) =>  this.removeRecreatedArtifact(measurement.artifact)
      )
    }
  }
...
createArtifact(artifact: Artifact) {
  if (artifact.measurementType === "Distance") {
    this._measurement = new Ruler(this, null);
    this._measurement.widget._handles[0].worldPosition = artifact.metadata[0];
    this._measurement.widget._handles[1].worldPosition = artifact.metadata[1];
    this._measurement.widget._label.style.display = "block";
    ...
    this._measurement.artifact = artifact;
    this._measurement.artifact.elements = elements;

  } else if (artifact.measurementType === "Angle") {
    this._measurement = new Angle(this, null);
    this._measurement.widget._handles[0].worldPosition = artifact.metadata[0];
    this._measurement.widget._handles[1].worldPosition = artifact.metadata[1];
    this._measurement.widget._handles[2].worldPosition = artifact.metadata[2];
    this._measurement.widget._label.style.display = "block";
  ...
  } else if (artifact.measurementType === "Density") {
    this._measurement = new Voxelprobe(this, null);
    this._measurement.widget._handle.worldPosition = artifact.metadata[0];
    this._measurement.widget._label.style.display = "block";
    ...
    this._measurement = new Annotation(this, null);
    this._measurement.widget._handle.worldPosition = artifact.metadata[0];
    this._measurement.widget._label.innerHTML = artifact.elements[1].innerHTML;
    this._measurement.widget._label.style.display = "block";
  ...
  this.domElement.removeEventListener("contextmenu", this._measurement.onContextMenu);
  this._measurement.artifact.recreated = true;

  // if(this.settings.isEducationMode) {
  //   this._measurement.artifact.education = true;
  // }

  if (!this.createArtifactMP) {
    this._measurements.push(this._measurement);
  }

  this.settings.canvas._measurements.push(this._measurement);
  this._measurement.widget.endAll();
}
