...
export class MenuBarComponent implements OnInit {
  ...
  constructor(
    ...
    this.userService.getAllGraphsStudy().pipe(first()).subscribe(graphs => {
      this.graphs = graphs;
    });
    this.userService.getAllStoriesStudy().pipe(first()).subscribe(stories => {
      this.stories = stories;
    });
    this.userService.getAllTextReportsStudy().pipe(first()).subscribe(textReports => {
  ...
  oneGraph(){
    this.settings.isComparisonMode = false;
    this.settings.isEducationMode = false;
  }
