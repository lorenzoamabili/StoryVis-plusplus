...
export class SlideDeckVisualization {
    ...
    constructor(slideDeck: IProvenanceSlidedeck, elm: HTMLDivElement) {
        ...
        d3.select("#slideDeck")
            ...
            .text("\uf0c1")
            .on("click", () => {
                d3.selectAll('.slide').each((slide: any) => slide.transitionTime = slide.transitionTime + 100);
                this.update();
            });
    ...
    private selectSlide = (slide: IProvenanceSlide | null) => {
        ...
        (window as any).tree._viz.traverser.graph.current = slide.node;
        (window as any).tree._viz.update();
    ...
    public onAdd = (node?: ProvenanceNode) => {
        ...
        let nodeSlide = node ? node : (window as any).tree._viz.traverser.graph.current;
    ...
    private shrink = () => {
        this.getDeck().slides.forEach((slide: any) => {
    ...
    public createStoryFromDerivationNodes() {
        let nodes = (window as any).tree._viz.traverser.graph.getNodes();
        var arrayNodes: any[] = [];
